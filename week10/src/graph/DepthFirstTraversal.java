...
public class DepthFirstTraversal<T> extends AdjacencyGraph<T> implements Traversal<T> {
    ...
    private ArrayDeque<T> backLog = new ArrayDeque<T>(); // the backlog
    ...
    public List<T> traverse() throws GraphError {
        ...
        while (node != null) { // while there is at least one unvisited node
            backLog.add(node); // add the unvisited node to the to do list
            traverseBackLogList(); // and traverse the to do list
    ...
    private boolean visited(T node) {
        return
                traversal.contains(node) // the node has been visited and is in the traversal
                        || backLog.contains(node); // or it is in the to do list, and will therefore be visited
    }
     ...
     */
    private void traverseBackLogList() throws GraphError {
        while (backLog.size() > 0) {
            T node = backLog.getLast();
            backLog.remove();
            visitNode(node);
        }
    }
